...
function add(add1, add2) {
  let addNum1 = Number(add1)
  let addNum2= Number(add2)
...
let movieChoice = bestMovie("Tron")
...
function theEliminator(contestants, loser) {
  for(i = 0; i < contestants.length; i++) {
    if(contestants[i] === loser) {
      ...
      console.log(`${loser} has been eliminated.`)
    }
  }
  return contestants
}
...
function emailCheck(email) {
  let emailUpdated = String(email.trim())
  if(emailUpdated.includes("@")) {
    console.log("email verified")
  } else {
    console.log("must provide a valid email address")
  }
}
emailCheck("hellohello.com")
...
function chocFrogs(gold) {
  console.log(`You can buy ${gold / 3} chocolate frogs`)
}
let totalFrogs = chocFrogs(17)
...
function chocFrogs2(gold) {
  if(gold % 3 === 0) {
    ...
    console.log(`You can buy ${(gold - 2) / 3} chocolate frogs with 2 gold left over`)
  } else{
    console.log(`You can buy ${(gold - 1) / 3} chocolate frogs with 1 gold left over`)
  }
}
let totalFrogs2 = chocFrogs2(16)
